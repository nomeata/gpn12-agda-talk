...
toℕ-injective {suc n} {suc i} {suc i'} eq = cong suc (toℕ-injective (cong pred eq))
...
mod-suc n refl | .(q * 7) | result q .zero = mod-lemma q 6 (suc zero)


mod-pred : ∀ n →  suc n mod 7 ≡ suc zero → n mod 7 ≡ zero
mod-pred n eq with n divMod 7
mod-pred .(toℕ r + q * 7) eq | result q r with toℕ r ≤? 5
mod-pred .(toℕ r + q * 7) eq | result q r | yes p  = toℕ-injective eq4 
  where r' = fromℕ≤ {suc (toℕ r)} {7} (s≤s (s≤s p))
        ...
        eq4 = cong pred $ begin
          ...
          toℕ (suc (zero {7}))
            ≡⟨ refl ⟩
          suc zero ∎
mod-pred .(toℕ r + q * 7) eq | result q r | no ¬p with eq3
  where eq2 = begin
          6
...
lem-sub-p : ∀ n p → (suc n mod 7 ≡ suc zero) → p < 6 → ((suc n ∸ suc p) mod 7 ≢ (suc zero))
lem-sub-p n zero eq1 _ eq2 with begin zero ≡⟨ sym (mod-pred n eq1) ⟩ n mod 7 ≡⟨ eq2 ⟩ suc zero ∎
... | ()
lem-sub-p n (suc p) eq <6 eq2 with n divMod 7 | mod-pred n eq 
lem-sub-p .0 (suc p) _ <6 () | result zero .zero  | refl
lem-sub-p .(7 + (q * 7)) (suc p) _ (s≤s (s≤s (<4))) eq2 | result (suc q) .zero | refl = ⊥-elim $ 1+n≰n 1<1
  where <7 : suc (6 ∸ p) ≤ 7
        <7 = s≤s (n∸m≤n p 6)
        eq4 = begin
            toℕ (fromℕ≤ <7) + q * 7
